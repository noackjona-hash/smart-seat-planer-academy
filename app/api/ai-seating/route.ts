...
export async function POST(request: Request) {
  try {
    ...
    // Use GEMINI_API_KEY for Vercel, fallback to the public one if needed for AI Studio preview
    const apiKey = process.env.GEMINI_API_KEY || process.env.NEXT_PUBLIC_GEMINI_API_KEY;
    
    if (!apiKey) {
      return NextResponse.json({ error: "Kein Gemini API Key gefunden (GEMINI_API_KEY fehlt in den Vercel Environment Variables)." }, { status: 500 });
    }
...
`;
    ...
    console.error("AI Seating API Error:", error);
